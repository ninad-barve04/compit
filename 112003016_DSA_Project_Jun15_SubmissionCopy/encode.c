...
void encode_byte( int byte, FILE *op_fp, ProbabilityDistribution *pdistib){
    ...
    /*
     * Compute sub interval of characer(byte) based on the frequency table and cumulative 
     * frequency. 
     * upperbound = loweround + (range X frequency/cumulative_frequency)
     */
    ...
    /**
     * To maintain the accuracy, make sure the range length is big enough
     * so that subsequent subdivisions are correct
     */
    for(;;){
...
/**
 * @brief function to accumulate (buffer) the bits to be written to 
 * decoded file as integer bits. once 8 bits are accumulated, it is written out
 * @param onebit 
 * @param fp 
 */
